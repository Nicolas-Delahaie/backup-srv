...
A Raspberry Pi 2B running in the living room, primarily used as a backup target for the main home server (see the `personal-server` repo). Since the hardware is always on, it has also been put to work as an AirPlay 2 audio receiver for the living-room speakers, reporting playback metadata to Home Assistant.
...
The Pi's primary purpose is to back up the main server. The AirPlay receiver is a bonus use of otherwise-idle hardware.

## What the Pi runs

- **Backup target** — the main reason it exists, driven from the `personal-server` side.
